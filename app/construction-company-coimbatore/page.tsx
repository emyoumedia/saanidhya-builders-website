...
import { CheckCircle, Phone, ArrowRight, MapPin, Home, Building2,
      ...
      <div className="bg-cream min-h-screen">
        ...
        <section className="bg-navy pt-32 pb-20 relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 md:px-6 relative">
            <nav className="flex items-center gap-2 font-montserrat text-xs text-white/40 mb-6" aria-label="Breadcrumb">
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <span className="font-montserrat text-xs font-semibold text-orange uppercase tracking-widest px-3 py-1.5 rounded-full border border-orange/20 bg-orange/10">
                    Updated {dateStr}
                  </span>
                </div>
                ...
                <p className="font-montserrat text-lg text-white/70 mb-6 leading-relaxed">
                  {city.description} Get accurate construction cost estimates, material pricing, and expert builder consultation.
                </p>
...
<a href={`tel:${company.contact.phoneRaw2}`} className="btn-secondary text-sm">
  <Phone size={15} /> {company.contact.phoneDisplay2}
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-8">
                ...
                <div className="font-playfair font-bold text-white mb-1" style={{ fontSize: '3rem' }}>
                  ₹{min.toLocaleString()}
                </div>
                ...
                {/* Null-safe rating — only show if we have real reviews */}
                {company.stats.googleRating && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-montserrat text-xs text-white/50 ml-1">
                      {company.stats.googleRating}★ rated · {company.stats.reviewCount}+ reviews
                    </span>
                  </div>
                )}
              </div>
        ...
        <section className="container mx-auto px-4 md:px-6 py-16">
          <div className="text-center mb-10">
            <span className="font-montserrat text-xs font-semibold text-orange uppercase tracking-widest mb-3 block">
              Cost Breakdown
            </span>
          ...
          <div className="bg-white rounded-2xl p-8 border border-navy/5 shadow-sm mb-16">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {city.popularAreas.map((area) => (
                <div key={area} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-cream border border-navy/5">
          ...
          {/* Why choose us — uses real stats from company.json */}
          <div className="bg-navy rounded-3xl p-10 text-white mb-16">
            ...
            <p className="font-montserrat text-white/60 mb-8">
              {company.stats.yearsExperience} of experience, {company.stats.projectsCompleted} projects delivered —
              quality construction on time.
            </p>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                { icon: CheckCircle,   title: 'Transparent Pricing', desc: 'Fixed-price contracts with no hidden charges or surprises' },
                { icon: Clock,         title: 'On-Time Delivery',    desc: 'Milestone-based project management with weekly photo updates' },
                { icon: Star,          title: `${company.warranty.structural} Warranty`, desc: 'Structural warranty on all projects, workmanship guarantee' },
                ...
                { icon: MessageSquare, title: 'Dedicated Manager',   desc: 'Single point of contact and regular progress updates' },
            ...
            <div className="space-y-4 max-w-3xl mx-auto">
              {[
                ...
                },
                {
                  ...
                  a: `A standard 1,200–1,800 sq.ft home in ${city.name} typically takes 12–18 months from foundation to handover. With ${company.name}'s dedicated project management, every milestone is tracked and communicated clearly.`,
                },
                {
                ...
                <details key={i} className="bg-white rounded-2xl border border-navy/5 shadow-sm group">
                  <summary className="flex justify-between items-center px-6 py-5 cursor-pointer font-montserrat font-semibold text-navy text-sm list-none">
                    {q}
                    <span className="text-orange ml-4 text-xl leading-none group-open:rotate-45 transition-transform inline-block flex-shrink-0">+</span>
                  </summary>
                  <div className="px-6 pb-5 font-montserrat text-sm leading-relaxed" style={{ color: 'rgba(11,15,59,.68)' }}>
                    {a}
                  </div>
                </details>
              ))}
              ...
              {cities
                ...
                .map((c) => (
                  <Link key={c.slug} href={`/construction-cost/${c.slug}`}
                    className="flex items-center justify-between bg-white rounded-xl px-5 py-4 border border-navy/5 hover:border-orange/30 hover:shadow-md transition-all duration-200 group">
                    <div>
                      <div className="font-montserrat font-semibold text-navy text-sm group-hover:text-orange transition-colors">
                        {c.name}
                      </div>
        ...
        <section className="gradient-bg py-16">
          <div className="container mx-auto px-4 md:px-6 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Link href="/contact"
                className="bg-white text-navy font-montserrat font-semibold text-sm px-8 py-4 rounded-full hover:bg-cream transition-colors inline-flex items-center gap-2">
