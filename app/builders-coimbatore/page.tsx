...
export const metadata: Metadata = {
  ...
  description: `Top-rated builders in Coimbatore. ${company.name} — RERA registered, ${company.stats.projectsCompleted} projects, ${company.stats.yearsExperience} years experience, ${company.stats.googleRating}★ rating. Residential & commercial builders in Coimbatore.`,
  keywords: [
  ...
  openGraph: {
    ...
    description: `Top-rated builders in Coimbatore. ${company.stats.projectsCompleted} projects, ${company.stats.yearsExperience} years, ${company.stats.googleRating}★ Google rating. RERA registered. Free consultation.`,
    url: `${company.website}/builders-coimbatore`,
    images: [{ url: '/images/og-image.jpg', width: 1200, height: 630, alt: 'Builders in Coimbatore' }],
  },
}

const statValueMap = (key: string) => {
  const map: Record<string, string> = {
    ...
    googleRating: company.stats.googleRating + '★',
  }
  return map[key] ?? key
}
...
const orgSchema = {
  ...
  '@type': 'Organization',
  name: company.name,
  description: `Best builders in Coimbatore with ${company.stats.yearsExperience} years experience and ${company.stats.projectsCompleted} completed residential and commercial projects.`,
  url: company.website,
...
export default function BuildersCoimbatorePage() {
  const { highlights, testimonials } = localSeo.buildersCoimbatore
  return (
    <>
      ...
      <section className="bg-navy pt-32 pb-20 relative overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 md:px-6 text-center">
          ...
          <p className="font-montserrat text-white/60 text-lg max-w-2xl mx-auto mb-8">
            {company.name} — {company.stats.yearsExperience} years of trust, {company.stats.projectsCompleted} projects delivered,
            and a {company.stats.googleRating}★ reputation in {company.serviceArea.city}.
          </p>
            ...
            {highlights.map((h) => {
              ...
              return (
                <div key={h.label} className="bg-white rounded-2xl p-6 shadow-sm border border-navy/5 text-center hover:shadow-md transition-shadow">
                  ...
                  <div className="font-playfair font-bold text-navy text-2xl mb-1">{statValueMap(h.stat)}</div>
